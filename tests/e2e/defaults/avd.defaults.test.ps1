...
<#
.SYNOPSIS
    Pester v5 post-deployment assertions for the AVD 'defaults' e2e scenario.
    Environment variables injected by the CI workflow:
      TEST_SUBSCRIPTION  — Azure subscription ID
      TEST_RG            — Resource group name (dep-avdNNNN-avd-dfl)
      TEST_NAME_PREFIX   — Module namePrefix used at deploy time (e.g. avdNNNNdfl)
#>

BeforeAll {
    $script:sub        = $env:TEST_SUBSCRIPTION
    $script:rg         = $env:TEST_RG
    $script:namePrefix = $env:TEST_NAME_PREFIX   # e.g. avd5432dfl
...
Describe 'Networking' {
    ...
    It 'Session host subnet (snet-avd-hosts) should exist' {
    ...
    It 'Private endpoint subnet (snet-avd-pe) should exist' {
        $subnet = $script:vnet.Subnets | Where-Object Name -eq 'snet-avd-pe'
        $subnet | Should -Not -BeNullOrEmpty
    }
}
...
Describe 'AVD Control Plane' {
    ...
    It 'Application group should be Desktop type' {
        $script:appGroup.ApplicationGroupType | Should -Be 'Desktop'
    }
    It 'Workspace <wsName> should exist' -TestCases @{ wsName = $script:wsName } {
        $script:workspace | Should -Not -BeNullOrEmpty
    }
    It 'Workspace should reference the application group' {
        $script:workspace.ApplicationGroupReference | Should -Contain $script:appGroup.Id
    }
}

Describe 'Session Hosts (deploySessionHosts=false in defaults)' {
    It 'No session hosts should be registered' {
        $sessionHosts = Get-AzWvdSessionHost -HostPoolName $script:hpName -ResourceGroupName $script:rg -ErrorAction SilentlyContinue
        $sessionHosts | Should -BeNullOrEmpty
    }
}
